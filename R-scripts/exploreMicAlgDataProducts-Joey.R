...
neonMicCol <- load("data-raw/MicroAlgae_Collection_NeonData.Robj")
neonMicCol <- NeonData
...
# write clean data
write_csv(afdm, "data-processed/microalgae_afdm.csv")
# write.csv(afdm,paste0(resultspath,'micAlg_afdm.csv'),row.names=F)


phytos <- afdm |> 
  filter(algalSampleType %in% c("phytoplankton", "seston")) |> 
  filter(aquaticSiteType == "lake") |> 
  mutate(afdm_per_volume = adjAshFreeDryMass/domainFilterVolume)
write_csv(phytos, "data-processed/phytos_afdm.csv")
